    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-5 shadow-xl">
        ...
        <ul className="mb-3 space-y-1">
          {rows.map(r => (
            <li key={r.p.id} className="flex items-center justify-between gap-2 rounded bg-emerald-50 px-3 py-2 text-sm">
              ...
              <span className="text-right text-xs text-emerald-700">
                {r.fits ? (
          ...
          {rows.length === 0 && <p className="text-sm text-emerald-700">Ninguna planta tiene frecuencia.</p>}
        </ul>
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <button onClick={onClose} className="rounded px-3 py-2 text-emerald-800 hover:bg-emerald-50">Cerrar</button>
          {!applied ? (
