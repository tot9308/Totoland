  ...
  return (
    <main className="min-h-screen bg-stone-50 p-4 md:p-8">
      <header className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-stone-800">🔔 Recordatorios</h1>
      ...
      {showForm && (
        <form onSubmit={save} className="mb-6 rounded-xl bg-[#faf7f0] p-4 shadow-sm">
          ...
          <div className="grid gap-2 md:grid-cols-2">
            ...
            <label className="block text-sm text-stone-800 md:col-span-2">
              Primera fecha
      ...
      {loading ? <p className="text-stone-700">Cargando…</p> : (
        <ul className="space-y-2">
          {items.map(r => (
            <li key={r.id}
              className={`rounded-xl p-3 shadow-sm ${r.active ? "bg-[#faf7f0]" : "bg-slate-100 opacity-60"}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <p className="text-sm font-medium text-stone-800">
                    🔔 {r.title}
                    {r.plant_id && (
                      <span className="ml-2 rounded bg-stone-100 px-1.5 text-[10px] text-stone-600">
                ...
                <div className="flex flex-col gap-1">
                  <button onClick={() => toggle(r)}
                    className="rounded bg-[#faf7f0] px-2 py-1 text-xs text-stone-600">
                    {r.active ? "⏸ Pausar" : "▶ Activar"}
                  </button>
                  <button onClick={() => openEdit(r)}
                    className="rounded bg-[#faf7f0] px-2 py-1 text-xs text-stone-600">✏️ Editar</button>
                  <button onClick={() => remove(r)}
                    className="rounded bg-[#b5603d] px-2 py-1 text-xs text-white">🗑 Borrar</button>
                </div>
              </div>
            </li>
          ))}
