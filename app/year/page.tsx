  ...
  return (
    <main className="min-h-screen bg-stone-50 p-4 md:p-8">
      <header className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-stone-800">📊 Tu año verde {new Date().getFullYear()}</h1>
      </header>
      ...
      {loading ? <p className="text-stone-700">Cargando…</p> : (
        <>
          ...
          <section className="mb-6 rounded-xl bg-[#faf7f0] p-4 shadow-sm">
            <h2 className="mb-3 text-lg font-semibold text-stone-800">Eventos por mes</h2>
            <div className="flex items-end gap-1" style={{ height: 120 }}>
              {stats.byMonth.map((v: number, i: number) => (
                <div key={i} className="flex flex-1 flex-col items-center gap-1">
                  <div
                    className="w-full rounded-t bg-stone-500"
                    style={{ height: `${(v / stats.maxMonth) * 100}%`, minHeight: v > 0 ? 4 : 0 }}
                    title={`${MESES[i]}: ${v}`}
                  />
                  <span className="text-[10px] text-stone-600">{MESES[i]}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          {stats.topSpecies.length > 0 && (
            <section className="mb-6 rounded-xl bg-[#faf7f0] p-4 shadow-sm">
              <h2 className="mb-2 text-lg font-semibold text-stone-800">Especies más regadas</h2>
          ...
          <section className="rounded-xl bg-[#faf7f0] p-4 shadow-sm">
            <h2 className="mb-2 text-lg font-semibold text-stone-800">Resumen</h2>
...
function Stat({ label, value, icon }: { label: string; value: number; icon: string }) {
  return (
    <div className="rounded-xl bg-[#faf7f0] p-4 text-center shadow-sm">
