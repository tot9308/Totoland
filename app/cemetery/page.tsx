  ...
  return (
    <main className="min-h-screen bg-stone-50 p-4 md:p-8">
      ...
      {loading ? (
        ...
        <p className="text-stone-700">Cementerio vacío. Que siga así 🌿</p>
      ) : (
        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {items.map(p => (
            <article key={p.id} className="rounded-xl bg-[#faf7f0] p-4 shadow">
              {p.thumbUrl && (
              ...
              <p className="text-sm text-stone-600">{p.species ?? "—"} · {p.location ?? "sin ubicación"}</p>
              ...
              {p.notes && <p className="mt-2 text-sm italic text-stone-600">“{p.notes}”</p>}
